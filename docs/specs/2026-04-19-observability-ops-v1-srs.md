...
- 批准记录: `docs/verification/spec-approval-phase2-observability-ops-v1.md`
...
- Given 业务代码调用 `logger.info` 时，`context` 字段只允许下列受控键集合：`{ traceId, module, event, actionName, route, method, status, durationMs, code, errorClass, userIdHash, role, workId, creatorId, postId, threadId, slowQueryMs, queryName, backupFile, scriptName }`。When logger 序列化该日志，Then 不在该集合中的键应被丢弃，且整体单条 log 体积不超过 8 KiB；超过则截断 `error.stack` 与 `context` 末尾，并附 `truncated=true` 字段。
...
- Given `ERROR_REPORTER_PROVIDER=sentry` 且 `SENTRY_DSN` 已设置，但 V1 内置 `sentry` provider 仅为占位、底层 SDK 未打包，When 系统初始化 reporter，Then 系统应记录一条 `warn` 日志说明「`sentry` provider 在 V1 仅占位，已降级到 `noop`」，并使用 `noop` provider，不抛出启动错误，也不发起任何网络请求。
...
### FR-006 环境变量契约扩展
#### 需求说明
系统应在 `web/src/config/env.ts` 中显式声明本增量引入的 env 字段，提供类型校验与默认值；env 字段缺省时不应阻塞启动，但应有可读的降级行为。本增量新增的字段集合为：`OBSERVABILITY_LOG_LEVEL`、`OBSERVABILITY_METRICS_ENABLED`、`OBSERVABILITY_METRICS_TOKEN`、`OBSERVABILITY_SLOW_QUERY_MS`、`ERROR_REPORTER_PROVIDER`、`SENTRY_DSN`、`SQLITE_BACKUP_DIR`。

#### 验收标准
- Given 默认配置（无新增 env 字段），When 系统启动，Then 实际行为：`OBSERVABILITY_LOG_LEVEL=info`、`OBSERVABILITY_METRICS_ENABLED=false`、`OBSERVABILITY_SLOW_QUERY_MS=100`、`ERROR_REPORTER_PROVIDER=noop`、`SQLITE_BACKUP_DIR` 未设置（备份脚本拒绝运行，`/api/health.backup.ready=false`）。
...
- Given 配置 `OBSERVABILITY_SLOW_QUERY_MS` 为非正数或非整数（如 `0`、`-50`、`abc`），When 系统启动，Then 应回退到默认 `100`，并产生一条 `warn` 级别启动日志说明降级原因。
...
- Given 当前应用实例正在写入 `community.sqlite`，When 备份脚本运行，Then 不应阻塞 / 锁死正在运行的实例。验收探测口径：在备份脚本运行的同时，对 `/api/health` 以 1Hz 顺序探测，应每秒至少有 1 次返回 HTTP 200 且 P95 RTT 不超过 2 秒；备份过程中不允许出现连续 2 秒以上的不可用窗口（必要时使用在线备份 API 或 WAL checkpoint，由设计阶段定结论）。
...
#### 验收标准
- Given `NODE_ENV=production`、`OBSERVABILITY_LOG_LEVEL=info`、`OBSERVABILITY_METRICS_ENABLED=true`，When 通过指定基准协议（详见下条）测量本增量前后 P95 时延差，Then 该差值不超过 5ms。
- 基线采集协议：在同一台开发机上，分别 checkout `master` 基线提交与本增量最终提交；每次启动后等待 10 秒预热，对 `/api/health` 顺序发起 1000 次同步请求（单连接，间隔 0），记录 P95；将两次 P95 之差作为净增。验收以本地基准为准，不要求与生产复现严格一致。

#### Priority
Should

#### Source / Trace Anchor
ROADMAP §3.8「指标 + 日志」隐含的"不影响线上性能"约束；增量记录 §影响矩阵「medium 影响、不动业务」。
...
#### Source / Trace Anchor
ROADMAP §1「保持 repository 边界」、§3.8 与 §6 风险章节中的"服务商选型未定"；增量记录 §变更包/明确不做 中"不接入真实 Sentry/外部上报后端"。
...
#### Source / Trace Anchor
ROADMAP §3.8「指标 / 错误上报」与 §2 指导原则「不引入伪装能力」；阶段 1 已确立的会话与 cookie 安全边界（README §2「账号与会话」）。
...
#### Source / Trace Anchor
ROADMAP §3.8「让生产环境可调试、可恢复、可演练」隐含的"可观测能力不应反向影响可用性"；增量记录 §变更包/Modified 中 env 契约扩展。
...
#### Source / Trace Anchor
ROADMAP §2 指导原则「质量门禁不变 / Keep the quality chain」与本增量 FR-009 的可观察行为；保持现有 Vitest 链路不被新接入打断。

## 8. 外部接口与依赖

- `IFR-001` `/api/metrics` HTTP 接口：方法 `GET`，需要 `Authorization: Bearer` 头，输出 JSON；与现有 `/api/discovery-events` `/api/health` 共用 Next.js App Router route handler 体系。Priority: Must；Source: ROADMAP §3.8「指标」、本规格 FR-005、CON-002。
- `IFR-002` `/api/health` 扩展字段：保留现有字段，扩展 `observability` 与 `backup` 命名空间。Priority: Should；Source: 本规格 FR-008、阶段 1 已上线的 `/api/health`（README §1）。
- `IFR-003` `web/scripts/backup-sqlite.mjs` / `web/scripts/restore-sqlite.mjs`：作为独立 CLI 入口，由运维人员手动或 cron 调用；CLI 自身使用相同 logger 与错误归一化。Priority: Must；Source: ROADMAP §3.8「备份与恢复演练」、本规格 FR-007。

## 9. 约束与兼容性要求

- `CON-001` 不引入新的运行时 npm 依赖（见 NFR-002）；只允许 Node.js 22+ 内置模块。Priority: Must；Source: NFR-002、增量记录 §变更包/明确不做。
- `CON-002` `/api/metrics` 在 `OBSERVABILITY_METRICS_ENABLED=false` 时必须**完全不存在**（HTTP 404），不允许返回 401；以避免向外暴露能力指纹。Priority: Must；Source: 本规格 FR-005 验收第 2 条、NFR-003。
- `CON-003` 现有数据库 schema 不允许在本增量中变更（与持久化迁移分离，留给后续 §3.1 增量）。Priority: Must；Source: ROADMAP §3.1（持久化迁移属于另一增量）、增量记录 §变更包/明确不做。
- `CON-004` 现有 UI 与 server action 的对外行为契约不允许在本增量中变更；只允许在边界处增加横切能力。Priority: Must；Source: 增量记录 §变更包/明确不做、§影响矩阵「无失效任务」。
- `CON-005` 所有结构化日志必须可被 `JSON.parse` 解析（生产模式）；`dev` 模式可以是可读文本。Priority: Must；Source: 本规格 FR-002 验收第 2 条。
